                ...
                <template #title="{ dataRef }">
                  ...
                  <div v-else class="datasheet-item flex flex-row items-center p-1 h-10">
                    ...
                    <div class="datasheet-item-action ml-auto">
                      <form-outlined class="m-2" @click.stop="onEdit(dataRef, 'rename')"/>
                      <delete-outlined @click.stop="datasheet.edit.delete(dataRef)"/>
                    </div>
                  </div>
                </template>
              </a-directory-tree>
          ...
          <div class="layout-main">
            <a-tabs ref="tabsRef" v-model:activeKey="view.state.id" destroyInactiveTabPane type="editable-card"
              ...
              <a-tab-pane v-for="(pane, index) in view.state.list" :key="pane.id"
                ...
                <component
                    :height="$route.query?.code ? 370 : 750" :is="view.components[pane.type]"
  ...
  setup() {
    ...
    const view = {
      ...
      async get(id) {
        ...
        if (data?.value) {
          const viewId = store.viewID ? store.viewID : data.value.rows?.[0]?.id;
