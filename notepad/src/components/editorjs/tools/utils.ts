...
export const getBolckId = (editor: API) => {
...
export class SelectionUtils {
    ...
    isAtEditor() {
        ...
        let selectedNode: any = (selection.anchorNode || selection.focusNode);
    ...
    static get rect() {
        let sel: any, range;
        // @ts-ignore
        const {createTextNode, createElement, selection} = document;
        sel = selection;
    ...
    setFakeBackground() {
        document.execCommand(this.commandBackground, false, '#1890ff');
...
export const singleClick = (fn, delay = 500) => {
    let timer = null;
    return function () {
        if (timer) {
            clearTimeout(timer);
