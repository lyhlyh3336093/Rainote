...
export default class Image extends BaseClass {
    nodes: {
    ...
    CSS = {
        wrapper: ['ce-block-image-wrapper', 'tc-wrap']
    }
    ...
    save(content) {
        if (store.image[store.id][this.block.id]) {
            return store.image[store.id][this.block.id]
        }
        return {}
    }
}