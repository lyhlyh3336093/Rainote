...
- ~~**列名净化 round-trip 断裂**~~（已修复）：导出端 `SqlIdentifierSanitizer` 净化列名（空格→`_` 等）曾使含特殊字符列名 round-trip 静默丢单元格。**修复模式**：`buildContext` 两遍注册——第一遍无条件注册全部原名键，第二遍把每列的净化名（`sanitize` 剥离反引号）作为兜底键 `putIfAbsent` 注册；原名键优先与列遍历顺序无关，同净化名多列（导出端歧义）取 sort 首列。真名列与别名净化名共存时（如 "col name" 与 "col_name"）SQL 列命中真名列。
...
- **对称契约是导入正确性的根**：反转义表、首列 record_id 锚点、净化列名三处契约均已对称（净化名兜底键使第三处 round-trip 闭合）。新导入能力会**显性化导出端既有缺陷**——评审发现的 SQL 导出双列错位（双列占行数据两单元格但渲染器按单列消费，首个双列后全部值错位；Excel 渲染器同族问题为双列后普通列被跳过）已修复：双列展开为 `名_ID`/`名_文本` 两列 + 值循环用独立数据索引消费，并用渲染产物 → SqlInsertParser 解析的 round-trip 测试锚定。教训：**遍历"列定义列表"消费"扁平行数据"时，列索引与数据索引在首个变长列后即错位，必须用独立数据索引**。
...
- 排除列（如 type=21 双向链接列）导出时展开为 `关联_ID`/`关联_文本` 两列（值为 linkRecordId 与关联记录名）；导入时目标表映射排除该类型（且无同名普通列）→ 双端一致跳过，link 数据不回写（R16）。
